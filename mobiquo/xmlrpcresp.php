...
function login_func()
{
    ...
    $response = new xmlrpcval(array(
        ...
        'max_avatar_width'  => new xmlrpcval($modSettings['avatar_max_width_upload'] ? $modSettings['avatar_max_width_upload'] : 10000, 'int'),
        'max_avatar_height' => new xmlrpcval($modSettings['avatar_max_height_upload'] ? $modSettings['avatar_max_height_upload'] : 10000, 'int'),
...
function get_topic_func()
{
    ...
    foreach($context['topics'] as $topic) {
        ...
        $xmlrpc_topic = new xmlrpcval(array(
            ...
            'short_content'     => new xmlrpcval(basic_clean($topic['first_post']['preview'], 200, 1), 'base64'),
            'icon_url'          => new xmlrpcval($avatar, 'string'),
...
function get_user_reply_post_func()
{
    ...
    foreach($context['posts'] as $post)
    {
        ...
        $xmlrpc_post = new xmlrpcval(array(
            ...
            'short_content'     => new xmlrpcval(basic_clean($post['body'], 200, 1), 'base64'),
...
function get_unread_topic_func()
{
    ...
    foreach($context['topics'] as $topic)
    {
        ...
        $xmlrpc_topic = new xmlrpcval(array(
            ...
            'reply_number'      => new xmlrpcval($topic['replies'], 'int'),
            'view_number'       => new xmlrpcval($topic['views'], 'int'),
            'short_content'     => new xmlrpcval($topic_info['last_body'], 'base64'),
            'icon_url'          => new xmlrpcval($topic_info['last_poster_avatar']),
...
function get_latest_topic_func()
{
    ...
    foreach ($context['posts'] as $topic)
    {
        $xmlrpc_topic = new xmlrpcval(array(
            ...
            'reply_number'      => new xmlrpcval($topic['replies'], 'int'),
            'view_number'       => new xmlrpcval($topic['views'], 'int'),
            'short_content'     => new xmlrpcval($topic['preview'], 'base64'),
            'post_author_id'    => new xmlrpcval($topic['poster']['id']),
            'post_author_name'  => new xmlrpcval(basic_clean($topic['poster']['name']), 'base64'),
            'new_post'          => new xmlrpcval($topic['is_new'], 'boolean'),
            'post_time'         => new xmlrpcval($topic['time'], 'dateTime.iso8601'),
    ...
    if (isset($context['get_topics']))
    {
        while ($topic = $context['get_topics']())
        {
           ...
           'post_author_name'       => new xmlrpcval(basic_clean($topic['matches'][0]['member']['name']), 'base64'),
                'short_content'     => new xmlrpcval(basic_clean($topic['matches'][0]['body'], 200, 1), 'base64'),
...
function m_get_report_post_func()
{
    ...
    {
        $matches = array();
        preg_match('/topic=(\d+)\.msg(\d+)/i', $report['topic_href'], $matches);
        ...
        $avatar = '';
        if (!isset($user_profile[$report['author']['id']]))
            loadMemberData($report['author']['id']);
        $profile = $user_profile[$report['author']['id']];
        if (!empty($settings['show_user_images']) && empty($profile['options']['show_no_avatars']))
        {
            $avatar = $profile['avatar'] == '' ? ($profile['id_attach'] > 0 ? (empty($profile['attachment_type']) ? $scripturl . '?action=dlattach;attach=' . $profile['id_attach'] . ';type=avatar' : $modSettings['custom_avatar_url'] . '/' . $profile['filename']) : '') : (stristr($profile['avatar'], 'http://') ? $profile['avatar'] : $modSettings['avatar_url'] . '/' . $profile['avatar']);
        }
        ...
        $post_list[] = new xmlrpcval(array(
            'report_id'         => new xmlrpcval($report['id']),
            'forum_id'          => new xmlrpcval($board_id),
            'forum_name'        => new xmlrpcval(basic_clean($board_name), 'base64'),
            'topic_id'          => new xmlrpcval($topic_id),
            'topic_title'       => new xmlrpcval(basic_clean($report['subject']), 'base64'),
            'post_id'           => new xmlrpcval($matches[2]),
            'post_title'        => new xmlrpcval(basic_clean($report['subject']), 'base64'),
            'post_author_name'  => new xmlrpcval(basic_clean($report['author']['name']), 'base64'),
            'post_author_id'    => new xmlrpcval(basic_clean($report['author']['id']), 'string'),
            'icon_url'          => new xmlrpcval($avatar),
            'post_time'         => new xmlrpcval($report['time_started'], 'dateTime.iso8601'),
            'timestamp'         => new xmlrpcval($report['timestamp_started'], 'string'),
            'short_content'     => new xmlrpcval(basic_clean($report['body'], 100, 1), 'base64'),
            'can_delete'        => new xmlrpcval(allowedTo('delete_any'), 'boolean'),
            'reported_by_id'    => new xmlrpcval($report['comments'][0]['member']['id']),
            'reported_by_name'  => new xmlrpcval(basic_clean($report['comments'][0]['member']['name']), 'base64'),
            'report_reason'     => new xmlrpcval(basic_clean($report['comments'][0]['message']), 'base64'),
        ), 'struct');
